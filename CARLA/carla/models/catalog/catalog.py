...
class MLModelCatalog(MLModel):
    ...
    Parameters
    ----------
    data : Union[DataCatalog, OnlineCatalog] Class
    ...
    def __init__(
        self,
        data: Union[DataCatalog, OnlineCatalog],
        model_type: str,
        backend: str,
        cache: bool = True,
        ...
        **kws,
        ...
        if isinstance(data, OnlineCatalog):
            ...
            if model_type == "forest":
                ...
                self._feature_input_order = list(np.sort(data.continuous + encoded_features))
    ...
    def predict_proba(
        ...
        if self._backend == "pytorch":
            ...
            if isinstance(x, pd.DataFrame):
                # todo: object type among the x columns
                has_object_type = x.dtypes[x.dtypes == 'object'].any()
                if has_object_type:
                    obj_type_columns = x.select_dtypes(include='object').columns.to_list()
                    x = x.astype({i: 'float64' for i in obj_type_columns})
                _x = x.values
