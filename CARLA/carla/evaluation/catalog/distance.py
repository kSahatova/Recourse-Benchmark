...
def mahalanobis_distance(factuals: np.ndarray, counterfactuals: np.ndarray):
    """
    ...
    cov_matrix = np.cov(factuals, rowvar=False)
    ...
    for original, counterfactual in zip(factuals, counterfactuals):
        distance = mahalanobis(original, counterfactual, inv_cov_matrix)
...
class Distance(Evaluation):
    ...
    def __init__(self, mlmodel, reduction=None, mah_factuals=None):
        super().__init__(mlmodel)
        self.columns = ["L0_distance", "L1_distance", "L2_distance", "Linf_distance"]
        self.reduction = reduction
        self.mah_factuals = mah_factuals
    ...
    def get_evaluation(self, factuals, counterfactuals):
        ...
        arr_f_p = self.mlmodel.get_ordered_features(self.mah_factuals).to_numpy()
        arr_cf = self.mlmodel.get_ordered_features(
            counterfactuals_without_nans
        ).to_numpy()

        distances = _get_distances(arr_f, arr_cf)
        distances_df = pd.DataFrame(distances, columns=self.columns)

        md = mahalanobis_distance(arr_f_p, arr_cf)
        md_df = pd.DataFrame(md, columns=["Mahalanobis_distance"])

        result = pd.concat([distances_df, md_df], axis=1)
        if self.reduction:
            try:
                if self.reduction == 'mean':
                    result = pd.DataFrame(result.mean(axis=0)).transpose()
            except ValueError:
                print('This type of reduction has not been implemented yet.')
